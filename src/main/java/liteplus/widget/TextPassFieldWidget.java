...
public class TextPassFieldWidget extends TextFieldWidget {
	...
	private liteplus.utils.FabricReflect FabricReflect;
	...
	public void renderButton(MatrixStack matrix, int mouseX, int mouseY, float delta) {
		...
		try {
			liteplus.utils.FabricReflect.getField(TextFieldWidget.class, "field_2092", "text").set(this, new String(new char[realText.length()]).replace("\0", "\u2022"));
		...
		try {
			liteplus.utils.FabricReflect.getField(TextFieldWidget.class, "field_2092", "text").set(this, realText);
		} catch (IllegalArgumentException | IllegalAccessException e) {
			System.err.println("Error reflecting text");
		}
	}
